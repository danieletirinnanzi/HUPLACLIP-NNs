...
# - presents it a batch of graphs (only with the clique if "only_clique_graphs = True", otherwise both with and without clique), contextually measuring model's output (can be correct/incorrect) and the input's Moran's I
...
for graph_size in config["graph_sizes"]:
    ...
    for model_specs in config["models"]:     
        ...
        save_path = os.path.join(os.getcwd(),"nns_moran_results", "fc-range", "CLIQUE" if only_clique_graphs else "BOTH", f"N{graph_size}",f"{model_name}_N{graph_size}_moran_results{'_CLIQUE' if only_clique_graphs else ''}.csv")
